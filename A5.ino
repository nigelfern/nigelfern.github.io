/*
 * Nigel Fernandes
 * A5 Higher Voltage and Transistors
 * HCDE 439
 * 2/22/21

Changes LED brightness based on received signals from Remote. Turns on motor when remote's Power button is pressed
...
int IR_RECEIVE_PIN = 10; //receives signals at 10
const int motorPin =  3; // motors signals at pin 3
const int ledPin = 6; // LED Pin is set to 6

void setup() {
  pinMode(6, OUTPUT); //initializes pin 6 for output
...
void loop() {
    ...
    if (IrReceiver.decode()) {
        switch (IrReceiver.decodedIRData.command) {//Decoded result is in the IrReceiver.decodedIRData structure.
          ...
          digitalWrite(motorPin, HIGH); //turns on motor
             break;
